...
void setup() 
{
  ...
  { 
    ...
    float n2=.5;
    float q=30;
    float bq=2*dist (0, 0, width, height);
    for (float p=q/2; p<bq; p=p+q)
    {
      stroke(lerpColor(icolour1, icolour2, map(p, 0, bq, 0, 1)));
      //fill(lerpColor(icolour1, icolour2, map(p, 0, dist (0, 0, i, j), 0, 1)));
      strokeWeight(20);
      smooth();
      line(n1*width+p-bq/2, n2*height-bq/2, n1*width+p+bq/2, n2*height+bq/2);
      line(n1*width-p-bq/2, n2*height-bq/2, n1*width-p+bq/2, n2*height+bq/2);
      line(n1*width+p+bq/2, n2*height-bq/2, n1*width+p-bq/2, n2*height+bq/2);
      line(n1*width-p+bq/2, n2*height-bq/2, n1*width-p-bq/2, n2*height+bq/2);
    }
