...
{
  //background(255);
  // size( 7860/3, 4320/3);//size( 8192/4, 4320/4);
  size( 7860/4, 4320/4);//size( 7860/3, 4320/3);
  noSmooth();
  //  col(); 
  //back();
  {
    noFill();
    strokeWeight(3);
    for (float w=width/9; w<width; w++) {
      for  (float v=0; v<height; v++) {
        // stroke(255); 
        stroke(lerpColor(color(255, 255, 0, random(10, 20)), color(255, 0, 100, random(150, 180)), map(dist(0, 0, w, v), 0, dist(0, 0, width, height), 0, 1)));
        point(w, v);
      }
    }
  }
