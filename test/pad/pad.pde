...
void setup() 
{
  ...
  for (float y1=-y0; y1<=y0; y1+=y0)
  {
    ...
    { 
       ...
       */      ellipse(i+x1, j+y1, 2*dist(0, 0, 7860, 4320)/20, 2*dist(0, 0, 7860, 4320)/20);
      stroke(255);
      int c=20;
      strokeWeight(c);
      noFill();
      ...
      ellipse(i+x1, j+y1, 2*dist(0, 0, 7860, 4320)/20-sqrt (2)*c, 2*dist(0, 0, 7860, 4320)/20-sqrt (2)*c);


      sentence=""+int(letters[no])+"";
      no++;
      //font1=loadFont("AgencyFB-Bold-255.vlw");
      //textFont(//font1, dist(0, 0, 7860, 4320)/13);
  ...
  //textFont(//font1, dist(0, 0, 7860, 4320)/90);
