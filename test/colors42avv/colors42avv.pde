...
void setup() 
{
  ...
  size( 7860/3, 4320/3);//size(7680, 4320);
  //background(20); 
  //fill(255);  
   ...
   }*/
  {
    ...
    float i, j, xx=.3, yy=.5, zx=height*.3, zy=height*.3, ny, nx, n=max( min(zx, zy)/2.5, 2);
    ny=n;
    nx=n;
    for ( j=height*yy- zy+ny/2; j <height*yy+zy; j+=ny) {
      for ( i=width*xx- zx+nx/2; i <width*xx+zx; i+=nx) { 
        //stroke(sen(i, j, xx-zx/width/2, yy-zy/height/2, xx+zx/width/2, yy+zy/height/2)); 
        strokeWeight(max( min(width, height)/15, 10)); 
        stroke(sen(i, j, xx-zx/width/1.5, yy-zy/height/1.5, xx, yy));  
        point(i, j);


        float cc=max( min(width, height)/100, 10), t=cc;
        stroke(250);
        strokeWeight(t);  
        strokeCap(ROUND);
        line(i-cc, j-cc, i+cc, j+cc);// up to down  
        line(i-cc, j+cc, i+cc, j-cc);// down to up
      }
    }
  }
  noStroke();
...
color sen(float i, float  j, float v1, float v2, float v3, float v4) {   
  ...
  color icolour2=color( 255, 255, 255);
