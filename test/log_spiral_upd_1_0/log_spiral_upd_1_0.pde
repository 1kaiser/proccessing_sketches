...
//PFont //font;
//PImage //img;
color  colour1= color(255, 100, 0, 200), ////background upper gradient
  colour2= color(255, 0, 0, 200);
void setup() {
  ////background(255);
  //size( 7860/3, 4320/3);//size( 7860/3, 4320/3);
  //img = loadImage("Sketch30220553.jpg");
  size( 7860/3, 4320/3);//size(  7860/3, 4320/3 );
  //background(255);
  ...
  //filter(GRAY);
  // loadPixels();
  for (int i1 = 0; i1 < 7860/3; i1++) {
    for (int j1 = 0; j1 <  4320/3; j1++) {
      if
        (
        (hue(get(i1, j1))>50)
        &&(saturation(get(i1, j1))>50)
        &&(brightness(get(i1, j1))>50)
        )//&&(green(get(i1, j1))>125))//&&(blue(get(i1, j1))==0))
        set(i1, j1, color(0));
    }
  }
  updatePixels();
  filter(GRAY);
  setGradient(0, 0, 7860/3, 4320/3, colour1, colour2);
    //blend(//img, 0, 0, 7860/3, 4320/3, 0, 0, 7860/3, 4320/3, DARKEST);
  ...
  /*
  float i=7860/3*.90;
   float j=4320/3/2;//-7860/3*.10;
   float y2=min(7860/3, 4320/3)/5*sqrt(2);
   char[] letters = {
   //  'K', 'A', 'I', 'S', 'E', 'R', 'R', 'O', 'Y',
   //  5, 2, 4, 7, 3, 7, 7, 6, 9
   9, 6, 7, 7, 3, 7, 4, 2, 5
