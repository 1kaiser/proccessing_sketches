...
void setup()
{
  ...
  //setGradient(0, 0, 7860, 4320, c1, c2);// //background 
  ...
  int no=0;
  /*  float y0=2*dist(0, 0, 7860, 4320)/15;
   ...
   //textFont(//font, dist(0, 0, 7860, 4320)/13);
   textAlign(CENTER, CENTER);
   fill(255, 255, 255);
   noSmooth();
   text(sentence, 7860/2+x1, 4320/2+y1);
   }
   }
   */
  //int no=0;
  float i=7860*.90;
  float j=4320/2;//-7860*.10;
  float y2=min(7860, 4320)/5*sqrt(2);
  float y0=2*y2/15;
  for (float y1=-y0; y1<=y0; y1+=y0)
  {
    ...
    { 
      ...
      noSmooth();
      // ellipse(7860/2, 4320/2, 2*sqrt(2)*250*sqrt (2), 2*sqrt(2)*250*sqrt (2));//central circle
      ...
      noSmooth();
      //noSmooth();
      /*      for (float n=0; n<dist (0, 0, 7860, 4320)/20; n++)
       {
  ...
  ////textFont(//font, dist(0, 0, 7860, 4320)/90);
  textAlign(CENTER, BOTTOM );
  fill(255);
  noSmooth();
  text(sentence, 7860/2, 4320);


  save("IMG_20140914_131831b.jpg");
}
  ...
  {
    {
      ...
      for (j=4320/2-3*n; j >0; j-=3*n) {
        for (i=7860/2+3*n; i <7860; i+=3*n) {
          quadrant_compoents();
        }
      }
    }

    {
      ...
      for (j=4320/2; j >0; j-=3*n) {
        for (i=7860/2; i >0; i-=3*n) {
          if (j==4320/2&&i==7860/2)
          {
            i-=3*n;
          }
          quadrant_compoents();
...

    {
      ...
      for (j=4320/2+3*n; j <4320; j+=3*n) {
        for (i=7860/2-3*n; i >0; i-=3*n) {
          quadrant_compoents();
        }
      }
    }

    {
      ...
      for (j=4320/2; j <4320; j+=3*n) {
        for (i=7860/2; i <7860; i+=3*n) {
          quadrant_compoents();
        }
      }
    }
