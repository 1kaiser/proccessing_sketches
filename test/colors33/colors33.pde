...
void setup() 
{
  ...
  {
    noSmooth();
    String s = "“ We can create magnificent worlds. We are all going to die anyway. Do you want to die for something or do you want to die for nothing? ”";
    fill(50);
    float n1=.5, n2=.4;
    ...
    fill(5, 242, 219);
    noStroke();
    // rect( n1*width-m*s.length()/2/2/2, n2*height-2*m/2, m*s.length()/2/2, 2*2* m );
    stroke(255); 
    fill(240);
    strokeWeight(min(width, height)/100);

    textAlign(CENTER, CENTER);
    text(s, n1*width-m*s.length()/2/2/2, n2*height-2*m/2, m*s.length()/2/2, 2*2*m);
  } 
