...
void setup() {
  ...
  for (float n=-30; n>-360; n=n-120)
  {
    ...
    for (float m=0+l; m<180+l; m=m+.1)
    {
      float b1=a1+2*s1*cos(radians(m));
      float b2=a2+2*s1*sin(radians(m));

      stroke(lerpColor(icolour1, icolour2, map(abs(m), abs(0+l), abs(180+l), 0, 1)));
      vertex(b1, b2);///
    }
    p++;
  }
